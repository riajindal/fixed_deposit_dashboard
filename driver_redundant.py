...
df_icici = pd.read_csv('icici_slabs.csv')
df_kotak = pd.read_csv('kotak_slabs.csv')
df_hdfc = pd.read_csv('hdfc_slabs.csv')
df_axis = pd.read_csv('axis_slabs.csv')
df_idfc = pd.read_csv('idfc_slabs.csv')
...
def bucket_slabs(df_1, df_2):
    ...
    for i in range(max(len(bank_2), len(bank_1_min))):
        if i >= len(bank_2):
            break
        if i < len(bank_1_min) and (bank_1_min[i] <= bank_2[i] < bank_1_max[i] or abs(bank_1_min[i] - bank_2[i]) <= 3):
...
binned_idfc = bucket_slabs(df_kotak, df_idfc)
binned_hdfc = bucket_slabs(df_kotak, df_hdfc)
binned_axis = bucket_slabs(df_kotak, df_axis)
binned_icici = bucket_slabs(df_kotak, df_icici)


# Custom function to get average interest after binning
def get_average_interest(binned, df):
    ...
    for key, values in res.items():
        res[key] = [df.loc[df['Tenure'] == value, 'General Rate'].values[0] for value in values]
...
binned_idfc = get_average_interest(binned_idfc, df_idfc)
binned_hdfc = get_average_interest(binned_hdfc, df_hdfc)
binned_axis = get_average_interest(binned_axis, df_axis)
binned_icici = get_average_interest(binned_icici, df_icici)

# print(len(binned_axis))
# print(binned_icici)
...
result = df_kotak
result = add_bank_slabs(binned_idfc, 'IDFC', result)
result = add_bank_slabs(binned_hdfc, 'HDFC', result)
result = add_bank_slabs(binned_axis, 'AXIS', result)
result = add_bank_slabs(binned_icici, 'ICICI', result)
# print(result.tail())
